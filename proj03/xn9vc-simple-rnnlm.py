...
########################################################
# since it's hard to get the key by value in trn_dict,
# we want to
reversed_trn_dict = {}
for key, value in trn_dict.items():
    reversed_trn_dict[key] = value
### write the log prob of testing to file
tst_out = open('xn9vc-tst-log-probs.txt', 'w')
start = time.time()
log_probs = 0
for data in tst_data:
    with torch.no_grad():
        inputs = torch.tensor(data[0], dtype=torch.long).to(device)
        results = torch.tensor(data[1], dtype=torch.long).to(device)
        score = rnn(inputs)
    for j in range(len(inputs)):
        log_prob = score[j][results[j]]
        #writing each word in each sentence to txt file
        line = reversed_trn_dict[results[j].item()] + "\t" + str(log_prob.item()) + "\n"
        tst_out.write(line)

end = time.time()
tst_out.close()
print("writing testing log probabilities in {} minutes".format((end - start) / 60))